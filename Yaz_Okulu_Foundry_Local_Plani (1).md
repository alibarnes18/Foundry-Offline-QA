...
## $100 Azure Kredisini Nerede & Nasıl Kullanacaksın?

> **Özet:** Proje çekirdeği (Foundry Local + SQLite + Python) tamamen yerel ve ücretsizdir — $100 kredi zorunlu değil. Ama **dört somut noktada** Azure'u kullanman mantıklı; aşağıda her birinde hangi haftada, hangi servisi, nasıl kullanacağın ve ne kadar harcayacağın adım adım yazılı.

---

### 1. Seçenek — Azure AI Foundry ile "Bulut vs. Yerel" Karşılaştırması
**Ne zaman:** Hafta 4 (LLM entegrasyonu bittikten hemen sonra)
**Neden:** Öğrencilerin Foundry Local'de yerel çalıştırdığı modelin aynısını bulutta çalıştırıp; hız, maliyet ve gizlilik farkını görmek. Bu hem CV'de iyi durur ("Azure AI Foundry deneyimi") hem de projeye güçlü bir "bulut vs. yerel" karşılaştırma bölümü ekler.

**Nasıl kullanacaksın:**
1. [portal.azure.com](https://portal.azure.com) → **Azure AI Foundry** servisini aç
2. Yeni bir **Project** oluştur → **Model Catalog**'dan Phi-3.5 Mini'yi seç → **Deploy**
3. Sana bir `endpoint URL` ve `API key` verecek
4. `src/llm.py` dosyasına ikinci bir fonksiyon ekle:

```python
import requests

def answer_query_cloud(user_question: str, context: str) -> str:
    """Aynı soruyu Azure AI Foundry endpoint'i üzerinden yanıtlar."""
    headers = {
        "Content-Type": "application/json",
        "api-key": "AZURE_API_KEY_BURAYA"
    }
    body = {
        "messages": [
            {"role": "system", "content": f"Yalnızca şu bağlamı kullan:\n{context}"},
            {"role": "user", "content": user_question}
        ]
    }
    response = requests.post("AZURE_ENDPOINT_URL_BURAYA", json=body, headers=headers)
    return response.json()["choices"][0]["message"]["content"]
```

5. `main.py`'de her soru için hem `answer_query()` (yerel) hem `answer_query_cloud()` (bulut) çağır, iki yanıtı ve yanıt sürelerini yan yana yazdır
6. Farkları `tests/test_results.md`'ye kaydet

**Tahmini maliyet:** Phi-3.5 Mini gibi küçük modeller için 1.000–2.000 test isteği ~$2–5 tutar. $100 kredinin %5'i bile harcamaz.

---

### 2. Seçenek — Azure Static Web Apps ile Demo Günü Yayını
**Ne zaman:** Hafta 5 sonu veya Hafta 6 başı (demo gününden 1–2 gün önce)
**Neden:** Demo günü herkes kendi laptopundan çalıştırmak yerine, jüri/izleyiciler tek bir linke girip asistanı tarayıcıdan denesin. Profesyonel görünür, deployment deneyimi kazandırır.

**Nasıl kullanacaksın (Streamlit seçtiysen):**
1. `ui/streamlit_app.py` dosyasını GitHub'a push et
2. Azure Portal → **App Service** → **Create** → Runtime: Python 3.11
3. GitHub reposunu bağla (Deployment Center)
4. Azure otomatik olarak her push'ta yeniden deploy eder
5. Sana `https://rag-asistan-xxx.azurewebsites.net` gibi bir link verir — bunu demo günü paylaş

**Nasıl kullanacaksın (sadece HTML/JS seçtiysen):**
1. Azure Portal → **Static Web Apps** → **Create**
2. GitHub reposundaki `ui/web/` klasörünü kaynak olarak göster
3. Backend (Flask) için ayrı bir App Service gerekir; veya demo için backend'i yerel çalıştırıp ngrok ile geçici URL oluşturabilirsin (ücretsiz)

**Tahmini maliyet:**
- Static Web Apps → **Free tier** (ayda $0, öğrenci projeleri için yeterli)
- App Service B1 tier → ~$13/ay, demo için sadece 2–3 gün açık tutarsan ~$1

---

### 3. Seçenek — Azure VM ile Ortak Geliştirme Ortamı
**Ne zaman:** Hafta 1 (kurulum sırasında, sadece gerekirse)
**Neden:** Takımdaki bazı öğrencilerin laptopları zayıfsa (ör. 4GB RAM) veya Windows/macOS/Linux üçünü aynı anda test etmek istiyorsan, herkesin bağlanabileceği ortak bir bulut VM kurabilirsin.

**Nasıl kullanacaksın:**
1. Azure Portal → **Virtual Machines** → **Create**
2. Bölge: **West Europe** (Türkiye'ye en yakın, düşük gecikme)
3. Boyut: **Standard_B2ms** (2 vCPU, 8GB RAM) — Phi-3.5 Mini CPU'da rahatlıkla çalışır
4. OS: **Ubuntu 22.04 LTS**
5. SSH ile bağlan:
   ```bash
   ssh azureuser@<VM_IP>
   pip install foundry-local-sdk
   git clone https://github.com/KULLANICI/yerel-rag-asistani
   cd yerel-rag-asistani && python main.py
   ```
6. Kullanmadığın zamanlarda VM'i **Durdur (Stop/Deallocate)** — durdurulmuş VM için işlemci ücreti ödenmez, yalnızca disk ücreti (~$1–2/ay) alınır

**Tahmini maliyet:** Günde 8 saat, 4 hafta kullanım → B2ms ~$0.096/saat × 8 × 28 ≈ **~$21**

---

### 4. Seçenek — Azure Blob Storage ile Doküman Paylaşımı
**Ne zaman:** Hafta 3 başı (ingestion öncesi)
**Neden:** Takım büyükse (3+ kişi) herkesin aynı ham dokümanlarla başlaması gerekir. USB taşımak yerine dokümanları Azure Blob Storage'a koy, herkes `azcopy` ile çeksin.

**Nasıl kullanacaksın:**
1. Azure Portal → **Storage Accounts** → **Create** → LRS (Locally Redundant, en ucuz)
2. **Container** oluştur → `data/raw/` içindeki dokümanları yükle
3. Takım arkadaşları indirmek için:
   ```bash
   pip install azure-storage-blob
   python -c "
   from azure.storage.blob import BlobServiceClient
   client = BlobServiceClient.from_connection_string('BAGLANTI_DIZESI')
   container = client.get_container_client('documents')
   for blob in container.list_blobs():
       with open(f'data/raw/{blob.name}', 'wb') as f:
           f.write(container.download_blob(blob.name).readall())
   print('İndirildi.')
   "
   ```

**Tahmini maliyet:** 10 kısa doküman (~5MB) için depolama maliyeti ~**$0.00** (LRS fiyatı GB başına $0.018/ay)

---

### Toplam Bütçe Özeti

| Seçenek | Ne zaman | Hizmet | Tahmini Maliyet |
|---|---|---|---|
| Bulut vs. Yerel karşılaştırma | Hafta 4 | Azure AI Foundry (Phi-3.5 Mini) | ~$3–5 |
| Demo günü yayını | Hafta 6 | App Service B1 (2–3 gün) | ~$1 |
| Ortak geliştirme VM | Hafta 1–4 | Standard_B2ms Ubuntu (günde 8 saat) | ~$21 |
| Doküman paylaşımı | Hafta 3 | Blob Storage LRS | ~$0 |
| **Toplam** | | | **~$25–30** |

$100 kredinin yaklaşık **%25–30'unu** kullanırsın. Geri kalanı eğer sonraki projelerde Azure servislerini denemek istersen orada kalır (kredi 1 yıl geçerlidir).
